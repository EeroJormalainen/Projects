...
public:
    ...
    // Returns the point of the snake's head. MOVED to public for GUI-purposes
    const Point& getHead() const;
    // Returns the point of the snake's tail. MOVED to public for GUI-purposes
    const Point& getTail() const;

    // Generates random numbers used to move the food item to random locations. MOVED to public for GUI-purposes
    std::default_random_engine rng_;

    // Prints the gameboard and the snake in it.
    void print() const;

    std::deque<Point> getSnake(); // Functions for the GUI to receive the snake and
    Point getFood();              // food coordinates
